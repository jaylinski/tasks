...
<template>
	<NcActions type="tertiary" :force-menu="true" :title="t('tasks', 'Add reminder')">
		<template #icon>
			<Plus :size="20" />
		</template>
		...
		<template v-if="!isReminderMenuOpen">
			<NcActionButton v-for="(alarm, index) in absoluteAlarms"
				:key="index"
				:title="alarm.label"
				close-after-click
		...
		<template v-if="hasStartDate && startDateMenuIsOpen">
			...
			<NcActionSeparator />
			<NcActionButton v-for="(alarm, index) in startDateAlarms"
				:key="index"
				:title="alarm.label"
				close-after-click
				@click="onAlarmOptionClick(alarm)">
		...
		<template v-if="hasDueDate && dueDateMenuIsOpen">
			...
			<NcActionSeparator />
			<NcActionButton v-for="(alarm, index) in dueDateAlarms"
				:key="index"
				:title="alarm.label"
				close-after-click
				@click="onAlarmOptionClick(alarm)">
...
import {
	getDefaultAlarms,
	formatAlarm,
	getAlarmObjectFromTriggerTime,
	getAlarmObjectFromDateTime, getDefaultAbsoluteAlarms,
...
export default {
	...
	computed: {
		...
		absoluteAlarms() {
			return getDefaultAbsoluteAlarms(this.timeZone).map((defaultAlarm) => {
				const alarmObject = getAlarmObjectFromDateTime(defaultAlarm, this.timeZone)
